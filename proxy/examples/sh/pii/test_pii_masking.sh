...
# This script tests if PII masking is functional by directly calling the security module
...
echo "=== PII Masking Functionality Test ==="
echo "This script tests if the PII masking functionality is working correctly."
echo ""
...
# Create a Python script to test the PII masking
cat > test_pii_masking.py << 'EOF'
import sys
import os
import logging
...
# Run the Python script with the PII text
echo "Running PII masking test..."
cd /workspaces/SynthLang && python proxy/examples/sh/pii/test_pii_masking.py "$PII_TEXT"

# Clean up
rm -f test_pii_masking.py
