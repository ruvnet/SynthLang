...
# Combined PII Masking Example with Debug Output
# This script demonstrates a combination of PII masking techniques and shows debug info
...
echo "=== Combined PII Masking Example with Debug Output ==="
...
RESPONSE1=$(curl -s -X POST http://localhost:8000/v1/chat/completions \
...
echo "RESPONSE WITHOUT PII MASKING (FULL JSON):"
echo "$RESPONSE1" | jq
echo ""
...
RESPONSE2=$(curl -s -X POST http://localhost:8000/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $API_KEY" \
...
echo "RESPONSE WITH PII MASKING (FULL JSON):"
echo "$RESPONSE2" | jq
echo ""

echo "Note: Compare the two responses above, especially the debug.compressed_messages section."
echo "In the first response, the LLM should see the original PII."
echo "In the second response, the LLM should see masked PII placeholders."
echo ""
